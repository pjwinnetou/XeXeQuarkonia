       ...
       bool fixParameters=0  )
{
  ...
  else if ( collId == kAADATA) f1 = new TFile("skimmedFiles/yskimXeXe_Trig-L1DoubleMuOpen2016_OpSign_EP-OppositeHF_20171030163_unIdentified.root");
 ...
 // if (muPtCut>0) kineCut = kineCut + Form(" && (((pt1>%.2f) && (pt2>%.2f)) || ((pt2>%.2f) && (pt1>%.2f)))", (float) muPtCut1, (float)muPtCut2, (float)muPtCut1, (float)muPtCut2 );
/*  if ( (collId == kAADATA) || (collId == kPADATA) || (collId == kAAMC) || (collId == kPAMC) || (collId == kAADATACentL3) || (collId==kAADATAPeri) )
  ...
  */
  ...
  RooRealVar mean1s("m_{#Upsilon(1S)}","mean of the signal gaussian mass PDF",pdgMass.JPsi, pdgMass.JPsi -0.1, pdgMass.JPsi + 0.1 ) ;
  ...
  RooRealVar *nSig1s= new RooRealVar("nSig1s"," 1S signals",0,5000);
  RooRealVar *nSig2s= new RooRealVar("nSig2s"," 2S signals",-30,300);
  RooRealVar *nSig3s= new RooRealVar("nSig3s"," 3S signals",-30,300);
...
*/  //RooRealVar m_lambda("#lambda","m_lambda",  init_lambda, 0,25);
  ...
  */
  ...
  RooRealVar ch4_k3("ch4_k3","ch4_k3",0.1,-1.12,1.2) ;
  RooRealVar ch4_k4("ch4_k4","ch4_k4",-0.1,-1.,1.) ;
  RooRealVar exp_k1("exp_k1","exp_k1",5,0,1000);
  RooRealVar k1_const("k1_const","k1_const",0.16,0.02,1.33);
  RooChebychev * bkg = new RooChebychev("cPol2Bkg","Background",*(ws->var("mass")),RooArgSet(ch4_k1,ch4_k2));
  /*
...
*/
  ...
  reducedDS->plotOn(myPlot2,Name("dataHist"),MarkerSize(.8),DataError(RooAbsData::SumW2),XErrorSize(0),Binning(rb));
...
//  ws->pdf("model")->plotOn(myPlot2,Components(RooArgSet(*cb3s)),LineColor(kOrange+7),LineWidth(2),LineStyle(2));
  ...
  else if(ptLow == 2.5 && ptHigh==5) drawText(Form("%.1f < p_{T}^{#mu#mu} < %.1f GeV/c",ptLow,ptHigh ),pos_text_x,pos_text_y,text_color,text_size);
  else drawText(Form("%.1f < p_{T}^{#mu#mu} < %.1f GeV/c",ptLow,ptHigh ),pos_text_x,pos_text_y,text_color,text_size);
...
//  drawText("|#eta^{#mu}| < 2.4", pos_text_x,pos_text_y-pos_y_diff*3,text_color,text_size);
  drawText(Form("Centrality %d-%d%s",cLow/2,cHigh/2,perc.Data()),pos_text_x,pos_text_y-pos_y_diff*2,text_color,text_size);
...
*/
  ...
  TFile* outf = new TFile(Form("PAS_fitresults_jpsi_DoubleCB_%s.root",kineLabel.Data()),"recreate");
  outh->Write();
  c1->SaveAs(Form("PAS_fitresults_jpsi_DoubleCB_%s.pdf",kineLabel.Data()));
  c1->SaveAs(Form("PAS_fitresults_jpsi_DoubleCB_%s.png",kineLabel.Data()));
  c1->Write();
  ws->Write();
