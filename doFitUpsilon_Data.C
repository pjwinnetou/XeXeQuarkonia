...
/*  if ( (collId == kAADATA) || (collId == kPADATA) || (collId == kAAMC) || (collId == kPAMC) || (collId == kAADATACentL3) || (collId==kAADATAPeri) )
  ...
  */
  ...
  RooRealVar    sigma1s_1("sigma1s_1","width/sigma of the signal gaussian mass PDF",0.05, 0.01, 0.25);
  ...
  RooRealVar alpha1s_1("alpha1s_1","tail shift", 1.961 , 1.413,4.01);
  ...
  RooRealVar n1s_1("n1s_1","power order", 1.804 , 1.413, 3.41);
  ...
  alpha1s_1.setVal(1.8);alpha1s_1.setConstant();
  ...
  RooRealVar *nSig1s= new RooRealVar("nSig1s"," 1S signals",0,1000);
...
//  RooFitResult* fitRes2 = ws->pdf("model")->fitTo(*reducedDS,Save(), Hesse(kTRUE),Range(massLow, massHigh),Minos(0), SumW2Error(kTRUE));
  ...
  ws->pdf("model")->plotOn(myPlot2,Name("bkgPDF"),Components(RooArgSet(*bkg)),LineColor(kOrange+7),LineStyle(kDashed),LineWidth(2));
  ...
  drawText(Form("p_{T}^{#mu_{1}} > %.f GeV/c", muPtCut ), pos_text_x,pos_text_y-pos_y_diff*2,text_color,text_size);
  drawText(Form("p_{T}^{#mu_{2}} > %.f GeV/c", muPtCut ), pos_text_x,pos_text_y-pos_y_diff*3,text_color,text_size);
