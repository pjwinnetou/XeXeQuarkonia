...
/*  if ( (fileID == kPPMCUps1S) || (fileID == kPPMCUps2S) || (fileID == kPPMCUps3S) )  { 
  ...
  */
  ...
  if (fileID == kPADATA) {
    ...
    fname = "OniaForest_HI_merged.root";
    mytree->Add(fname.Data());
  }
  ...
  if (fileID == kPADATA) {
    ...
    newfile = new TFile(Form("skimmedFiles/yskimXeXe_Trig-%s_%s_EP-%s_%s_%s.root",trigName.Data(),fdimusign.Data(),epName.Data(), getDayAndTime().Data(), skimVersion.Data()),"recreate"); 
  }
  else if  (fileID == kAADATACentL3) {
